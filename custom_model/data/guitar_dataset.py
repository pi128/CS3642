# -*- coding: utf-8 -*-
"""
Guitar Dataset for Training Custom Model
Loads and processes guitar chord audio files for training.
"""
...
class GuitarChordDataset(Dataset):
    """Dataset for guitar chord transcription training."""
    ...
    def _load_samples(self) -> List[Dict]:
        ...
        for audio_file in audio_files:
            ...
            chord_key = audio_file.stem.lower().replace('__danglada__', '')
...
def create_dataloaders(data_dir: str, batch_size: int = 8, num_workers: int = 0) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """Create train and validation dataloaders."""
    
    # Create datasets
    train_dataset = GuitarChordDataset(data_dir, split='train', augment=True)
    val_dataset = GuitarChordDataset(data_dir, split='val', augment=False)
...
if __name__ == "__main__":
    ...
    print("🎸 Testing Guitar Chord Dataset")
    print("=" * 50)
    
    # Create dataset
    dataset = GuitarChordDataset(data_dir, split='train')
    ...
    if len(dataset) > 0:
        ...
        # Test dataloader
        train_loader, val_loader = create_dataloaders(data_dir, batch_size=2)
